<template>
    <div>
        ...
        <div>
            <label>Text:</label> <br/>
            <textarea v-model="text" style="width: 300px"></textarea>
        </div>

        <div v-for="(value,type) in convertResult">
        ...
        <div style="display: flex">
            <div style="margin-right: 5px">
                <label>Text:</label> <br/>
                <textarea v-model="encText" style="width: 300px"></textarea> <br/>
            </div>
            <div>
                <label>Key(Optional):</label> <br/>
                <textarea v-model="encKey" style="width: 100px"></textarea>
            </div>
        </div>


        <div v-for="(encType,i) in encTypes">
            <input type="checkbox" :value="encType.enable" :checked="encType.enable"
                   @input="encType.enable = !encType.enable;updateSelectedEncText(i)"/>
            <label>{{encType.type}}:</label> <span
                :style="{color: encType.error?'red' :'' ,'text-decoration': 'underline'}">{{encType.result}}</span>
        </div>

        <h1 class="underlined">Data Transfer▼</h1>
        <div>
            <toggle :options="['Binary Hex String','Base64Encode','Plain String','Int8Array']" label="Input:"
                    v-model="transferModel.inputMode"></toggle>
            <br/>
            <textarea v-model="transferModel.inputObject" style="width: 75%;height: 200px"></textarea>
        </div>
        <div>
        ...
        <div>
            <toggle :options="['Binary Hex String','Binary Quote String','echo -ne','Plain String','Int8Array']"
                    label="Output:"
                    v-model="transferModel.outputMode"></toggle>
            <br/>
            <textarea v-model="transferModel.outputObject" style="width: 75%;height: 200px"></textarea>
        </div>
    </div>
    ...
    module.exports = {
        ...
        data() {
            return {
                ...
                encKey: "",
                encTypes: [
                    {"type": "MD5", "uri": "/misc/md5?s=${text}", "error": false, "result": "", enable: true},
                    {
                        "type": "Hash",
        ...
        methods: {
            ...
            join(type, s) {
                if (type === "under_line") {
                    ...
                    return s.map(e => StringUtils.capitalize(e)).join("")
                }
            },
            updateEncText() {
                let cfg = {
            ...
            transfer() {
                if (this.transferModel.inputMode !== 'Binary Hex String' && this.transferModel.inputMode !== 'Plain String' && this.transferModel.inputMode !== 'Int8Array') {
                    alert("Only support 'Binary Hex String','Plain String','Int8Array' input currently")
                    return
                }
                ...
                if (this.transferModel.inputObject === null || this.transferModel.inputObject === '') {
                    return
                }
                // inputObject
                ...
                if (this.transferModel.inputMode === 'Binary Hex String') {
                    inputObject = DataUtils.decodeHexString(inputObject)
                ...
                if (this.transferModel.outputMode === 'Binary Quote String') {
                    this.transferModel.outputObject = DataUtils.quoteToCString(inputObject)
                } else if (this.transferModel.outputMode === 'echo -ne') {
                    this.transferModel.outputObject = "echo -ne '" + DataUtils.quoteToCString(inputObject) + "'"
                    ...
                    this.transferModel.outputObject = DataUtils.encodeToHexString(inputObject)
                } else if (this.transferModel.outputMode === 'Int8Array') {
                    this.transferModel.outputObject = DataUtils.toInt8ArrayJSON(inputObject)
                } else {
