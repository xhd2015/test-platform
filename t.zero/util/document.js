
export function moveSelection(pos) {
    if (window.getSelection()) {
        let anchor = window.getSelection().anchorNode
        window.getSelection().setPosition(anchor, pos == null ? anchor.length : pos)
    }
}

export function updateTextareaHeight(e) {
    e.style.height = 'auto';
    e.style.height = e.scrollHeight + 'px';
}
export function updateTextareaWidth(e) {
    e.style.width = 'auto';
    e.style.width = e.scrollWidth + 'px';
}
export function getCurrentUrlQuery() {
    return getUrlQuery(location.search)
}

export function getUrlQuery(query) {
    let pageObj = {}
    if (!query) {
        return pageObj
    }
    if (query.startsWith("?")) {
        query = query.substr(1)
    }
    let arr = query.split("&")
    for (let a of arr) {
        let eqIdx = a.indexOf("=")
        let key
        let value
        if (eqIdx === -1 || eqIdx === a.length) {
            key = a
        } else {
            key = a.substring(0, eqIdx)
            value = a.substring(eqIdx + 1)
        }
        if (value == null) {
            value = ""
        }
        key = decodeURIComponent(key)
        value = decodeURIComponent(value)
        let existing = pageObj[key]
        if (existing == null) {
            pageObj[key] = value
        } else if (existing.constructor === Array) {
            existing.push(value)
        } else {
            existing = [existing, value]
            pageObj[key] = existing
        }
    }
    return pageObj
}
export function getPrevSiblingBottom(e) {
    while (e && e.previousSibling) {
        if (e.previousSibling.offsetParent) {
            return e.previousSibling.offsetTop + e.previousSibling.offsetHeight
        }
        e = e.previousSibling
    }
    return 0
}
export function getMarginTopToPlaceThisElementOnParentBottom(e) {
    if (e.parentElement) {
        return e.parentElement.offsetHeight - getPrevSiblingBottom(e) - e.offsetHeight
    }
    return 0
}

export function placeToParentBottom(e) {
    let pos = getMarginTopToPlaceThisElementOnParentBottom(e)
    e.style.marginTop = pos + "px"
}

